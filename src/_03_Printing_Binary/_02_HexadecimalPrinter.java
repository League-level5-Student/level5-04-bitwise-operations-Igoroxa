...
public class _02_HexadecimalPrinter implements ActionListener {
	...
	String binaryToHex(String binaryStr) {
		Integer[] hex = new Integer[binaryStr.length()];
		int sum = 0;
		for (int i = 0; i < hex.length; i++) {
			hex[i] = Integer.parseInt(String.valueOf(binaryStr.charAt(i)));

		}
		
		for (int i = 0; i < hex.length/4; i++) {
			for (int j = hex.length; j > 0; j=j-4) {
			...
			String dec = binaryToDec(str.toString());
			
			int saved = Integer.parseInt(dec);
	...
	String binaryToDec(String binaryStr) {
		Integer[] one = new Integer[binaryStr.length()];
		int sum = 0;
		for (int i = 0; i < one.length; i++) {
			one[i] = Integer.parseInt(String.valueOf(binaryStr.charAt(i)));
		...
		multiplicant = 1;
		String finale = String.valueOf(sum);
